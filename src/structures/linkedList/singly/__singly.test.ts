//#region>Meta
/**
 * @file src/structures/linkedList/singly/__singly.test.ts
...
describe('Singly Linked List', () => {
  ...
  describe('Insertion', () => {
    ...
    it('before/after node by ID', () => {
      ...
      expect(updatedItems.map((i) => i.data)).toEqual(['A', 'D', 'B', 'E', 'C']);
    });
    it('edge cases', () => {
      const nodeA = createNode('A', 'id-a');
      const nodeB = createNode('B', 'id-b');
  ...
  describe('Access and Search', () => {
    beforeEach(() => {
      const nodes = [createNode('A', 'id-a'), createNode('B', 'id-b'), createNode('C', 'id-c')];
      nodes.forEach((node) => list.append(node));
    });
  ...
  describe('Movement Operations', () => {
    ...
    it('invalid to index', () => {
      ...
      expect(afterMoveItems.map((i) => i.data)).toEqual(initialItems.map((i) => i.data));
    ...
    describe('moveToTarget edge cases', () => {
      it('should handle moving to a non-existent target', () => {
        ...
        expect(afterItems.map((i) => i.data)).toEqual(beforeItems.map((i) => i.data));
      });
      ...
      it('should handle moving a node before the head', () => {
        ...
        expect(afterMoveBeforeHead.map((i) => i.data)).toEqual(['C', 'A', 'B', 'D']);
      });
      ...
      it('should handle moving a node after the head', () => {
        ...
        expect(afterMoveAfterHead.map((i) => i.data)).toEqual(['A', 'D', 'B', 'C']);
      });
    });
  });
