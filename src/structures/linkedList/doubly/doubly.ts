//#region>Meta
/**
 * @file src/structures/linkedList/doubly/doubly.ts
  ...
  *reverse() {
  ...
  insert({ indexOrId, node, position = 'before' }: Base.InsertArgs<T>) {
    if (typeof indexOrId === 'number') {
      ...
      if (position === 'before' && target.node === this._head) return this.prepend(node);
      if (position === 'after' && target.node === this._tail) return this.append(node);
      return this.insertAtTarget(target.node, node, position);
    }
  }
  remove(indexOrId: number | string) {
    if (!this._head) return this;
    const target = (typeof indexOrId === 'number' ? this.get(indexOrId) : this.find(indexOrId))!;
  ...
  // Edge cases nearly impossible to reach
  /* v8 ignore next 5 */
  get(index: number) {
    if (index < 0 || index >= this._size) return undefined;
    const direction = index < Math.round(this._size / 2) ? 'forward' : 'reverse';
  ...
  moveToTarget(nodeToMoveId: string, targetNodeId: string, position: 'before' | 'after') {
    ...
    if (position === 'before' && targetNode === this._head) return this.prepend(movingNode);
...
const create = <T>(
  ...
  reverse: () => Generator<{ data: T; id: string; index: number }, void, unknown>;
} => {
  const list = new Doubly<T>(nodes);
  const format = (node: undefined | DataNode.DoublyLinkedInstance<T>): Base.AccessorResult<T> =>
    node ? { data: node.data, id: node.id } : undefined;
  const api = Object.freeze({
    *forward() {
      for (const { node, index } of list.forward()) yield { data: node.data, id: node.id, index };
    },
    *reverse() {
      for (const { node, index } of list.reverse()) yield { data: node.data, id: node.id, index };
    },
    get head() {
      return list.head;
    },
    ...
    moveToTarget(nodeToMoveId: string, targetNodeId: string, position: Base.InsertPosition) {
