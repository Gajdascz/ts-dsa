//#region>Meta
/**
 * @file src/structures/node/node.ts
...
const create = Object.freeze({
  linked: <T>(args: Detail<T> & LinkedPointer<T>): Linked<T> => new Linked(args),
  doublyLinked: <T>(args: Detail<T> & DoublyLinkedPointers<T>): DoublyLinked<T> =>
